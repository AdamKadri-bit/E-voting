...
class ElectionReadinessService
{
    ...
    public function evaluate(Election $election): array
    {
        ...
        // 7. End-to-end elections need their threshold key before any ballot can be encrypted.
        if ($election->isE2e()) {
            $done = $election->key_ceremony_status === 'complete' && $election->joint_public_key !== null;
            $checks[] = [
                'key' => 'key_ceremony',
                'label' => 'Trustee key ceremony complete',
                'passed' => $done,
                'detail' => $done
                    ? "Election key generated by {$election->trustee_count} trustees ({$election->trustee_threshold} needed to decrypt)."
                    : ($election->key_ceremony_status === 'failed'
                        ? 'The key ceremony failed after a trustee complaint; reset it and run it again.'
                        : 'Assign trustees and have each complete the key ceremony before activating.'),
            ];
        }
